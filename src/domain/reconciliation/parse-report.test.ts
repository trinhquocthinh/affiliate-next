...
describe("parseCommissionReport", () => {
  it("TC-099: Cột lạ ngoài 47 cột được bỏ qua, parse thành công", () => {
    const csvContent =
      `ID đơn hàng,Item id,Tên Item,Thời Gian Đặt Hàng,Trạng thái đặt hàng,Trạng thái sản phẩm liên kết,Giá(₫),Giá trị đơn hàng (₫),Hoa hồng ròng tiếp thị liên kết(₫),Sub_id1,Cột lạ 1,Cột lạ 2\n` +
  ...
  it("TC-101: Thiếu cột ID đơn hàng sẽ ném lỗi ERR_REPORT_FORMAT", () => {
    const csvContent =
      `Item id,Tên Item,Thời Gian Đặt Hàng,Trạng thái đặt hàng,Trạng thái sản phẩm liên kết,Giá(₫),Giá trị đơn hàng (₫),Hoa hồng ròng tiếp thị liên kết(₫),Sub_id1\n` +
      `123456,Sản phẩm 1,2026-08-10 12:00:00,Hoàn thành,Hoàn thành,100000,100000,10000,REQ-1`;

    expect(() => parseCommissionReport(csvContent)).toThrowError(
      'ERR_REPORT_FORMAT: Thiếu cột "ID đơn hàng"',
    );
  });

  it("Bỏ qua dòng rỗng hoặc dòng không có ID đơn hàng", () => {
    const csvContent =
      `ID đơn hàng,Item id,Tên Item,Thời Gian Đặt Hàng,Trạng thái đặt hàng,Trạng thái sản phẩm liên kết,Giá(₫),Giá trị đơn hàng (₫),Hoa hồng ròng tiếp thị liên kết(₫),Sub_id1\n` +
